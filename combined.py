...
if __name__ == '__main__':
    ...
    s='/home/pi/project/test_images/1i.jpg'
    ...
    while j<3:
        ...
        if GPIO.input(echo1)==1:
            ...
            if d1>20 and d1<200 and d2>20 and d2<200 :
                ...
                camera.close()
                numplate=start()
                curs.execute("INSERT INTO details (slot, name, contact, carnum) VALUES (%s,%s,%s,%s)",('1','ABC','9999999999',numplate))
                db.commit()
                db.close()
            ...
            #no car in slot1, moving to slot 2
            #sending pulse from u3
            GPIO.output(trig3,1)
            time.sleep(0.00001)
            GPIO.output(trig3,0)

            i=0
            while GPIO.input(echo3)==0 and i<350:
                st3=time.time()
                i+=1
            ...
            if GPIO.input(echo3)==1:
                ...
                #checking for slot 2
                if d3>20 and d3<200 and d4>20 and d4<200 :
                    print 'Distance3 is ',DISTANCE3,' cm'
                    print 'Distance4 is ',DISTANCE4,' cm'
                    ...
                    camera.close()
                    numplate=start()
                    curs.execute("INSERT INTO details (slot, name, contact, carnum) VALUES (%s,%s,%s,%s)",('2','XYZ','9898989898',numplate))
                    db.commit()
                    db.close()
